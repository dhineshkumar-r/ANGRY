...
def extract_features(tokenized_doc, config):
    ...
    features_object = FeatureGen(tokenized_doc, n_grams=config.n_grams, summary_len=config.summary_size,
                                 sim_th=config.similarity_score, max_th=config.max_sent_thresh,
                                 min_th=config.min_sent_thresh)
